...
| Authorization | a real permission model: grants scoped system-wide or to one activity, each linking any number of roles, and `system:administrator` as a bypass — **refused outright in an activity grant**, so a manager of one course never becomes an administrator of the installation |
...
| RustFS | **1.0.0** | the S3 suite's default endpoint, and the Compose stack |
| SeaweedFS | **4.47** | the S3 suite with `ALGOJUDGE_S3=seaweedfs`, **run by hand** — it skips by default, on CI included |
